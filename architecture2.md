...
- **`test_scenario_03` halt regex.** The reactive-path test was hardened (disallow `consult_director` for the test + assert on the `halt_intercepted` log event rather than a downstream deliverable), which required widening a halt-language pattern in `hooks/router.py`. The widening is global to the Stop-hook path; the false-positive cost is bounded (the `stop_hook_active` guard makes a spurious interception cost one extra brain call, not a loop). A full integration re-run to confirm no other scenario shifted has not yet been done.
